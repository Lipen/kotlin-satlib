    ...
    with(BDD(storageCapacity = 1 shl 23)) {
        ...
        for ((layerId, layer) in aig.layers.withIndex().drop(1)) {
            ...
            for (id in layer) {
                ...
                val leftVar = mkVar(id2var[gate.left.id]!!).let { if (gate.left.negated) -it else it }
                val rightVar = mkVar(id2var[gate.right.id]!!).let { if (gate.right.negated) -it else it }
                val def: BddRef = applyEq(mkVar(id2var[id]!!), applyAnd(leftVar, rightVar))
                // val node: BddRef = applyAnd(applyAnd(applyAnd(def, interval), left), right)
                // val node: BddRef = applyAnd(applyAnd(applyAnd(def, left), right), interval)
                val node: BddRef = applyAnd(def, applyAnd(left, right))
